...
class PageBot(webdriver.PhantomJS):
    ...
    def leer_articulos(self):
        ...
        if os.path.isdir('data/' + grav_date):
            print('Path already exist {}'.format(grav_date))
    ...
    def generate_date(self):
        years = ["{0:04}".format(i) for i  in range(2017, 2019)]
        months = ["{0:02}".format(i) for i in range(1, 12)]
        days = ["{0:02}".format(i) for i in range(1, 30)]
    ...
    def feed_date(self):
        ...
        for year in y:
            for month in m:
                for day in d:
                    if(month in ["01","02","03","04","05","06","07"])  and (year == "2017"):
                        pass
                    else:
